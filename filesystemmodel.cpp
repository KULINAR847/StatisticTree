...
QVariant FilesystemModel::data(const QModelIndex &index, int role) const
{
	...
	switch (index.column()) {
    case NameColumn:
        return nameData(fileInfo, role, nodeInfo->parent);
	default:
		break;
	}
	return QVariant();
...
QVariant FilesystemModel::nameData(const QFileInfo &fileInfo, int role, NodeInfo* parent) const
{
	switch (role) {
	case Qt::EditRole:
		return fileInfo.fileName();
	case Qt::DisplayRole:
        if (fileInfo.isRoot() || parent == 0) {
            ...
            return fileInfo.fileName(); //fileInfo.completeBaseName();
		}
...
QString FilesystemModel::getData(const QModelIndex &index, int role)
{
    if (!index.isValid()) {
        return "";
    }
    ...
    NodeInfo* nodeInfo = static_cast<NodeInfo*>(index.internalPointer());
    return nodeInfo->fileInfo.absoluteFilePath();
}
...
void FilesystemModel::fetchRootDirectory(QString dirPath)
{
    _nodes.clear();
    _nodes.append(NodeInfo(QFileInfo(dirPath)));
}
